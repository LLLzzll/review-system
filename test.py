...
def render_layout():
    ...
    with st.sidebar:
        ...
        with center_col:
            for name in tabs:
                if st.button(name, key=f"menu_{name}", use_container_width=True):
        ...
        subtabs = subtab_map.get(tab, [])
        if subtabs:
            subtab_key = f"subtab_{tab}"
            if subtab_key not in st.session_state:
                st.session_state[subtab_key] = subtabs[0]
            st.markdown("#### 分支")
            current_subtab = st.session_state[subtab_key]
            selected_subtab = st.selectbox(
                "",
                subtabs,
                index=subtabs.index(current_subtab),
            )
            st.session_state[subtab_key] = selected_subtab

    subtab_key = f"subtab_{tab}"
    current_subtab = st.session_state.get(subtab_key)
